...
export function ConversationItem({
  ...
  function saveRename() {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        setActionError(result.error ?? "Couldn't rename that chat.");
      }
    });
  }
  ...
  function confirmDelete() {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        setActionError(result.error ?? "Couldn't delete that chat.");
        setMode("idle");
      }
    });
  }
  ...
  if (mode === "renaming") {
    return (
      <div className="flex flex-col gap-1 rounded-lg bg-foam px-2.5 py-1.5">
        ...
        {actionError && <p className="px-1 text-xs text-red-600">{actionError}</p>}
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col">
      ...
      {actionError && <p className="px-2.5 pb-1 text-xs text-red-600">{actionError}</p>}
    </div>
  );
}
